...
// MockParticipantAntwoordRepositoryWithIDGeneration wraps the MockParticipantAntwoordRepository to add ID generation
type MockParticipantAntwoordRepositoryWithIDGeneration struct {
	*mocks.MockParticipantAntwoordRepository
}

// Create overrides the Create method to set an ID before saving
func (m *MockParticipantAntwoordRepositoryWithIDGeneration) Create(ctx context.Context, antwoord *models.ParticipantAntwoord) error {
	// Set a UUID for the antwoord
	antwoord.ID = "generated-id-" + time.Now().Format("20060102150405")
	return m.MockParticipantAntwoordRepository.Create(ctx, antwoord)
}

func TestAanmeldingHandler_ListAanmeldingen(t *testing.T) {
	// Setup
	mockDB := mocks.NewMockDB()
	mockAanmeldingRepo := mocks.NewMockParticipantRepository(mockDB)
	mockAanmeldingAntwoordRepo := mocks.NewMockParticipantAntwoordRepository(mockDB)
	mockAuthService := new(MockAuthService)
	mockPermissionService := mocks.NewMockPermissionService()

	// Create test aanmeldingen
	testAanmeldingen := []*models.Participant{
		{
			ID:        "1",
			Naam:      "Test User 1",
	...
	// Create handler
	mockEventRegRepo := mocks.NewMockEventRegistrationRepository(mockDB)
	handler := handlers.NewParticipantHandler(mockAanmeldingRepo, mockAanmeldingAntwoordRepo, (*services.EmailService)(nil), mockAuthService, mockPermissionService, mockEventRegRepo)
...
func TestAanmeldingHandler_GetAanmelding(t *testing.T) {
	...
	testAanmelding := &models.Participant{
		ID:        "test-id",
		Naam:      "Test User",
	...
	testAntwoord := &models.ParticipantAntwoord{
		ID:             "antwoord-id",
		ParticipantID:  testAanmelding.ID,
		Tekst:          "Test antwoord",
		VerzondOp:      time.Now(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedCode == fiber.StatusOK {
				...
				// Note: Status, Notities, BehandeldDoor, and BehandeldOp have been moved to EventRegistration model in V28
				// These fields are no longer part of the Participant model
			}
		})
	}
}
